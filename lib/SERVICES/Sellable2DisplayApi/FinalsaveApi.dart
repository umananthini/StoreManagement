...
class SellableFinalApi {
  int rescode = 500;
  static Future<finalsellableModel> getData(String? itemCode,String? serialNum,int? serialBatchQty,String? productSerialNo) async {
    try {
      log("GetOutward body:::${ConstantValues.token.toString()}");
      final responce = await http.post(
        Uri.parse(
            "${URL.queryApi}Inv/v1/AddSellable"),
        headers: {
      ...
      if (responce.statusCode == 200) {
        return 
        finalsellableModel.fromJson(
            json.decode(responce.body), responce.statusCode);
      } else {
        return 
        finalsellableModel.exception(
            json.decode(responce.body), responce.statusCode);
      }
    } catch (e) {
      log("GetOutwardcatch:::${e.toString()}");
      return 
      finalsellableModel.issues(e.toString(), 500);
    }
  }
}
