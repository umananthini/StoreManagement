...
class savefinalOutApi {
  static Future<finalsaveModel> getdata(
    ...
    int rescode = 500;
    try {
      log("${URL.queryApi}:::ConstantValues.token::" +
          ConstantValues.token.toString());
      final responce =
          await http.post(Uri.parse("${URL.queryApi}Inv/v1/AddStockOutward"),
              headers: {
...
class finalsaveModel {
  ...
  factory finalsaveModel.exception(Map<String, dynamic> json, int stcode) {
    return finalsaveModel(
        dataa: null, exception: json['respDesc'], stcode: stcode);
  }
...
class dataaa {
  ...
  factory dataaa.fromJson(Map<String, dynamic> jsons) {
    return dataaa(
        DocEntry: jsons["DocEntry"] ?? "", DocNumber: jsons["DocNumber"] ?? "");
  }
}
