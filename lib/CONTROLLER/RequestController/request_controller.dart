...
class RequestController extends ChangeNotifier {
  ...
  showRequestPopup(String cardCode, String value, String id, String approvalSp,
      String rejectSp, BuildContext context) {
    showDialog(
        context: context,
        builder: (_) {
...
// print("requestsData:${value.response},${requestsData}");
  ...
  getApproval(
    ...
    isApproval = true;
    notifyListeners();
    await RequestListApi.getApprovalData(cardCode, value, id, sp)
        .then((value) async {
      if (value.stcode! >= 200 && value.stcode! <= 210) {
        await fetchRequest();
  ...
  getReject(
    ...
    isReject = true;
    notifyListeners();
    await RequestListApi.getRejectData(cardCode, value, id, sp)
        .then((value) async {
      if (value.stcode! >= 200 && value.stcode! <= 210) {
        await fetchRequest();
        Get.snackbar(
